...
export const uploadFileUrl = {
  /** 用户头像上传地址 */
  USER_AVATAR: `${import.meta.env.VITE_SERVER_BASEURL}/file/upload`,
}
 ...
 * @param options 上传选项
 */
export function useFileUpload<T = string>(url: string,
  filePath: string,
 ...
 * @returns 上传状态和控制对象
 */
export function useUpload<T = string>(url: string,
  formData: Record<string, any> = {},
  ...
  const run = () => {
    ...
    uni.chooseImage({
      ...
      sourceType,
      success: (res) => {
        url = `${url}${pathExtName(res.tempFiles[0].name)}`
        // 开始上传
        loading.value = true
        progress.value = 0
        uploadFile<T>({
